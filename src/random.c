...
void random_init()
{
    ...
    uint64_t mangled_time = (current_time | (current_time << 32)) ^ ((uint64_t)clock() << 32);

    prng_state.s[0] ^= mangled_time;
    prng_state.s[1] &= mangled_time;
    prng_state.s[2] ^= mangled_time / 47;
    prng_state.s[3] += mangled_time;

    // Discard first 256 values generated by prng
    for (int i = 0; i < 256; ++i) {
        xoshiro256ss(&prng_state);
    }
}
